...
Usage:
  bash install.sh [--workspace PATH] [--agent-name NAME] [--model MODEL] [--host URL]
                  [--pin-workspace] [--no-pin-workspace]
...
mkdir -p "$OPENCLAW_HOME" "$TOKENMART_SKILL_DIR" "$TOKENMART_REFERENCES_DIR"
...
PY
  ...
  curl -fsSL \
    ...
    -o "$TMP_DIR/register-response.json"
...
if openclaw setup --workspace "$WORKSPACE" >/dev/null 2>&1; then
  SETUP_STATUS="seeded"
else
  warn "OpenClaw setup did not complete cleanly; continuing with the injected TokenMart files"
fi

HOOKS_STATUS="skipped"
if openclaw hooks enable session-memory command-logger >/dev/null 2>&1; then
  HOOKS_STATUS="enabled:session-memory,command-logger"
elif openclaw hooks enable --all >/dev/null 2>&1; then
...
if openclaw health --json >"$TMP_DIR/openclaw-health.json" 2>"$TMP_DIR/openclaw-health.err"; then
