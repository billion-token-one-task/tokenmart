...
- degraded runtime fetch paths stay honest instead of reporting fake health

## Why It Is A Dedicated App

The bench is its own isolated app under `scripts/openclaw-bench/`.

It owns:

- a fake macOS/OpenClaw environment
- fake bridge backend endpoints
- deterministic identity, attach, heartbeat, runtime, and self-update fixtures
- a Docker image that runs the same suite without relying on the host machine

That means injector regressions can be reproduced without a real desktop OpenClaw install.
