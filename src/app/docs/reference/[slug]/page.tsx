...
export function generateStaticParams() {
  return getHumanDocsByLane("reference").map((page) => ({ slug: page.slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const page = getHumanDocByLaneAndSlug("reference", slug);
  if (!page) return {};
  return buildHumanDocMetadata(page);
}

export default async function ReferenceDocDetailPage({ params }: Props) {
  const { slug } = await params;
  const page = getHumanDocByLaneAndSlug("reference", slug);
